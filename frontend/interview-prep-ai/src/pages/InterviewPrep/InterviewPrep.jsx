  ...
  async function fetchSessionDetailsById() {
    try {
      ...
      if (response.data && response.data.session) {
        console.log("Updated session data after pin toggle:", response.data.session);
    ...
    <DashboardLayout>
      ...
      <div className="container mx-auto pt-4 pb-4 px-4 md:px-0">
          ...
          <div className={`col-span-12 ${openLearnMore ? "md:col-span-7" : "md:col-span-8"}`}>
            <AnimatePresence>
              {sessionData?.questions?.map((data, index) => {
                //console.log("Rendering QuestionCard for index:", index, "data:", data);
                return (
                  <motion.div
                    key={data._id || index}
                  ...
                  >
                      <>
                        <QuestionCard
                          ...
                          answer={data?.answer}
                          openLearnMore={() => generateConceptExplanation(data.question)}
                          isPinned={data?.isPinned}
                          onTogglePin={() => toggleQuestionPinStatus(data._id)}
                      />
                      {!isLoading && sessionData?.questions?.length == index+1 && (
                        <div className='flex items-center justify-center mt-5'>
                              <button className='flex items-center gap-3 text-sm text-white font-medium bg-black px-5 py-2 mr-2 rounded text-nowrap cursor-pointer' disabled={isLoading || isUpdateLoader} onClick={uploadMoreQuestions}><LuListCollapse className='text-lg'/>{" "}Load More</button>
                          </div>
        ...
        <div>
            ...
            >
              {errorMsg && (
                <p className='flex gap-2 text-sm text-amber-600 font-medium'><LuCircleAlert className='mt-1'/>{errorMsg}</p>
              )}
